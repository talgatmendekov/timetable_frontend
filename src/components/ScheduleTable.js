...
// ─────────────────────────────────────────────────────────────────────────────
// Mobile card view (unchanged from original)
// ─────────────────────────────────────────────────────────────────────────────
const MobileView = ({
  ...
  return (
    <div>
      {daysToShow.map(day => {
        ...
        const classCount = groupsToShow.reduce((acc, g) =>
          acc + timeSlots.filter(tm => {
            const cls = getClass(g, day, tm);
            return cls && !cellsToSkip.has(`${g}-${day}-${tm}`);
          }).length, 0);
        ...
        return (
          <div key={day} className="mob-day-section">
            ...
            {!isCollapsed && groupsToShow.map(group => {
              ...
              if (!isAuthenticated) {
                const hasContent = slots.some(tm => getClass(group, day, tm) || getBooking(group, day, tm));
                ...
                <div key={group} className="mob-group-block">
                  ...
                  <div className="mob-slots">
                    {slots.map(tm => {
                      ...
                      let bkInfo   = null;
                      if (bk) {
                        bkBorder = bk.status === 'approved' ? '#22c55e' : bk.status === 'rejected' ? '#ef4444' : '#eab308';
                        bkInfo = (
                          <div className="mob-slot-booking" style={{ color: bkBorder }}>
                            {bk.status === 'approved' ? '✅' : bk.status === 'rejected' ? '❌' : '⏳'}
                            {' '}{bk.name || ''}{bk.room ? ` · ${bk.room}` : ''}
                          </div>
                        );
                      }
                      ...
                      return (
                        <div
                          ...
                          style={bkBorder ? { borderLeft:`3px solid ${bkBorder}` } : cd && ts ? { borderLeft:`3px solid ${ts.color}` } : {}}
                          onClick={() => {
                            if (isAuthenticated) { onEditClass(group, day, tm); return; }
                            if (!cd && !bk && onGuestBookCell) onGuestBookCell(group, day, tm);
                          }}
                        >
                          <div className={`mob-slot-time ${isToday ? 'today-t' : ''}`}>
                            {tm}
                            {cd && dur > 1 && <span className="mob-duration-badge">⏱ {dur * 40}m</span>}
                          </div>
...
const GCalPopup = ({ event, anchorRect, onClose, onEdit, isAuthenticated, typeLabels }) => {
  ...
  const HEADER_COLOR = { lecture:'#4285f4', lab:'#34a853', seminar:'#f9ab00' };
  const BK_COLOR     = { approved:'#34a853', rejected:'#ea4335', pending:'#f9ab00' };
  const typeKey   = cd?.subjectType || 'lecture';
  const headerBg  = bk ? (BK_COLOR[bk.status] || BK_COLOR.pending) : (HEADER_COLOR[typeKey] || HEADER_COLOR.lecture);
  ...
  if (top + PH > window.scrollY + window.innerHeight - 12)
    top = Math.max(window.scrollY + 12, window.scrollY + window.innerHeight - PH - 12);
    ...
    <div className="gcpop-overlay" onMouseDown={onClose}>
      <div ref={ref} className="gcpop" style={{ top, left }} onMouseDown={e => e.stopPropagation()}>
        ...
        <div className="gcpop-body">
          ...
          {isAuthenticated && (
            <button className="gcpop-edit-btn" onClick={() => { onEdit(); onClose(); }}>✏️ Edit class</button>
          )}
        </div>
      </div>
    </div>
...
// KEY DESIGN: rows = time slots, columns = days (one per day, not one per group)
//             multiple groups in the same day+time cell stack inside that cell
// ─────────────────────────────────────────────────────────────────────────────
const CalendarView = ({
  ...
  // Pastel colours matching screenshot 1 exactly
  const TYPE_COLORS = {
  ...
  // Compute actual calendar dates for each day column
  const weekDayLabels = useMemo(() => {
    ...
    return daysToShow.map((dayName, i) => {
      ...
      const shortName = (t(dayName) || dayName).slice(0, 3);
      return { dayName, label: `${shortName} ${d.getDate()}`, date: d };
    });
  ...
  const handleBlockClick = useCallback((e, group, day, time, cd, bk) => {
    e.stopPropagation();
    const r = e.currentTarget.getBoundingClientRect();
    setPopup({ event:{ classData:cd, booking:bk, group, day, time }, anchorRect:r });
  }, []);

  return (
    <div className="gcal-wrap">

      {/* Nav bar — matches screenshot 1 exactly */}
        ...
        <div className="gcal-grid" style={{ '--gcal-cols': daysToShow.length }}>
          ...
          {timeSlots.map(time => (
            <div key={time} className="gcal-row">
              ...
              {/* One cell per DAY (not per group) */}
              {daysToShow.map(day => {
                const isToday = day === todayName;

                /*
                 * Gather all events for this (day, time) across every group.
                 * They all land in the SAME column cell — that's what makes it
                 * a week grid, not a list.
                 */
                ...
                return (
                  ...
                  >
                    {blocks.map(({ group, cd, bk }) => {
                      const duration = Math.min(6, Math.max(1, parseInt(cd?.duration) || 1));
                      const colors   = bk
                      ...
                      return (
                        <div
                          ...
                          onClick={e => handleBlockClick(e, group, day, time, cd, bk)}
                        >
                          <div className="gcal-block-name">{cd?.course || bk?.purpose}</div>
                          {cd?.room && <div className="gcal-block-room">{cd.room}</div>}
                        </div>
                      );
                    })}
                  </div>
...
const ScheduleTable = ({
  ...
  const handleDragStart = (e, g, d, tm) => {
    setDragSource({ group:g, day:d, time:tm });
  ...
  const handleDragOver = (e, g, d, tm) => {
    e.preventDefault(); e.dataTransfer.dropEffect = 'move';
    if (!dragOver || dragOver.group !== g || dragOver.day !== d || dragOver.time !== tm)
      setDragOver({ group:g, day:d, time:tm });
  };
  const handleDragLeave = e => { if (!e.currentTarget.contains(e.relatedTarget)) setDragOver(null); };
  const handleDrop = (e, tg, td, ttm) => {
    e.preventDefault();
    if (!dragSource) return;
    const { group:fg, day:fd, time:ft } = dragSource;
    if (fg === tg && fd === td && ft === ttm) { handleDragEnd(); return; }
    moveClass(fg, fd, ft, tg, td, ttm);
    handleDragEnd();
  };

  // Legend + view toggle (shown above both table and calendar)
  const Legend = () => (
  ...
  return (
    <div className="schedule-container">
      ...
      {/* Mobile — always shown on small screens regardless of viewMode */}
      <MobileView
      ...
      {viewMode === 'calendar' && (
        <CalendarView
          daysToShow={daysToShow} groupsToShow={groupsToShow} timeSlots={timeSlots}
          schedule={schedule} todayName={todayName}
          weekOffset={weekOffset} onShiftWeek={setWeekOffset}
          cellsToSkip={cellsToSkip} occupiedRoomCells={occupiedRoomCells}
          selectedRoom={selectedRoom} normSelectedTeacher={normSelectedTeacher}
          isAuthenticated={isAuthenticated} bookings={bookings}
          onEditClass={onEditClass} onGuestBookCell={onGuestBookCell}
              ...
              {groupsToShow.map(group => (
                <tr key={group}>
                  ...
                  {daysToShow.map(day => timeSlots.map(tm => {
                    ...
                    const cd      = getClass(group, day, tm);
                    const show    = shouldShow(cd, day, tm);
                    const isToday = day === todayName;
                    const cf      = getConflicts(group, day, tm, cd);
                    ...
                    let bkLabel = null;
                    if (bk) {
                      bkStyle = bk.status === 'approved' ? { background:'#dcfce7', borderLeft:'3px solid #22c55e' }
                        : bk.status === 'rejected'       ? { background:'#fee2e2', borderLeft:'3px solid #ef4444' }
                        :                                  { background:'#fef9c3', borderLeft:'3px solid #eab308' };
                      bkLabel = (
                        <div style={{ fontSize:'0.68rem', marginTop:3, fontWeight:600,
                          color: bk.status === 'approved' ? '#166534' : bk.status === 'rejected' ? '#991b1b' : '#854d0e',
                    ...
                    return (
                      <td key={cellKey}
                        ...
                        colSpan={dur}
                        onClick={() => {
                          if (isAuthenticated && !dragSource) { onEditClass(group, day, tm); return; }
                          if (!isAuthenticated && !cd && !bk && onGuestBookCell) onGuestBookCell(group, day, tm);
                        }}
                      ...
                      >
                        {cd ? (
                          <div className="cell-content">
                            ...
                            {(cf.includes('teacher') || cf.includes('room')) && (
                              <div className="cell-conflict-icons">
                                {cf.includes('teacher') && <span>⚠️</span>}
                                {cf.includes('room')    && <span>🚪⚠️</span>}
                              </div>
                            )}
