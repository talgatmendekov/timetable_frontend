...
const endTime = (start, dur) => {
  try {
    ...
    const total = h * 60 + m + Math.min(300, Math.max(0, parseInt(dur) || 0));
    return `${String(Math.floor(total / 60) % 24).padStart(2,'0')}:${String(total % 60).padStart(2,'0')}`;
  } catch { return '??:??'; }
};
const toMinsLocal = (t) => { const [h,m] = (t||'0:0').split(':').map(Number); return h*60+m; };
const emptyForm  = () => ({ group_names:[], subject:'', teacher:'', room:'', exam_date:'', start_time:'9:00', duration:90, notes:'' });
    ...
    <div className="es-group-picker">
      <div className="es-gp-header">
        <span className="es-gp-count">
          {selected.length > 0
            ? `${selected.length} ${selected.length > 1 ? (t('groupsSelected') || 'groups selected') : (t('groupSelected') || 'group selected')}`
            : (t('noGroupsSelected') || 'No groups selected')}
        </span>
        <div className="es-gp-actions">
          <button type="button" className="es-gp-btn" onClick={() => onChange([...groups])}>{t('all') || 'All'}</button>
          <button type="button" className="es-gp-btn" onClick={() => onChange([])}>{t('clearFilters') || 'Clear'}</button>
        </div>
      </div>
...
export default function ExamSchedule({ readOnly = false, showExamsToGuests = false, setShowExamsToGuests = null }) {
  ...
  const handleSave = async () => {
    setError('');
    if (!form.group_names?.length)   return setError(t('errorSelectGroup')  || 'Select at least one group');
    if (!form.subject.trim())        return setError(t('errorSubjectRequired') || 'Subject is required');
    if (!form.room.trim())           return setError(t('errorRoomRequired')    || 'Room is required');
    if (!form.exam_date)             return setError(t('errorDateRequired')    || 'Date is required');
    if (!form.start_time)            return setError(t('errorTimeRequired')    || 'Start time is required');
    setSaving(true);
    try {
      ...
      const d = await r.json();
      if (!d.success) return setError(d.error || (t('saveFailed') || 'Failed to save'));
  ...
  const handleEdit = (exam) => {
    setForm({ group_names: exam.group_names||[], subject: exam.subject, teacher: exam.teacher||'', room: exam.room, exam_date: exam.exam_date?.slice(0,10)||'', start_time: exam.start_time, duration: exam.duration, notes: exam.notes||'' });
  ...
  const handleBroadcast = async (exam) => {
    ...
    try {
      ...
      const d = await r.json();
      if (d.success) setSendLog([`✅ ${t('sentTo')||'Sent to'} ${groupNames.length} ${t('groups')||'group(s)'} — ${d.sent} ${t('delivered')||'delivered'}, ${d.failed} ${t('failed')||'failed'}`]);
      else setSendLog([`❌ ${t('failed')||'Failed'}: ${d.error}`]);
    } catch (e) { setSendLog([`❌ ${t('error')||'Error'}: ${e.message}`]); }
  ...
  const dates    = useMemo(() => [...new Set(exams.map(e=>e.exam_date?.slice(0,10)).filter(Boolean))].sort(), [exams]);
  ...
  const byDate = useMemo(() => {
    const map = {};
    filteredExams.forEach(e => { const d = e.exam_date?.slice(0,10); if (!map[d]) map[d]=[]; map[d].push(e); });
    return map;
  }, [filteredExams]);
  ...
  const conflictIds = useMemo(() => {
    const ids = new Set();
    const byDateRoom = {};
    exams.forEach(e => { const k=`${e.exam_date}__${e.room}`; byDateRoom[k]=byDateRoom[k]||[]; byDateRoom[k].push(e); });
    Object.values(byDateRoom).forEach(group => {
      for (let i=0;i<group.length;i++) for (let j=i+1;j<group.length;j++) {
        const a=group[i],b=group[j],aS=toMinsLocal(a.start_time),aE=aS+a.duration,bS=toMinsLocal(b.start_time),bE=bS+b.duration;
    ...
    <div className="es-wrap" style={{ overflowX:'hidden', maxWidth:'100%', boxSizing:'border-box' }}>
        ...
        <div className="es-header-left">
          ...
          <div>
            ...
            <div className="es-sub">
              {exams.length} {exams.length !== 1 ? (t('exams') || 'exams') : (t('exam') || 'exam')}
              {conflictIds.size > 0
                ? ` · ⚠️ ${Math.floor(conflictIds.size/2)} ${t('roomConflicts') || 'room conflict(s)'}`
                : ` · ✅ ${t('noConflicts') || 'No conflicts'}`}
            </div>
          </div>
        </div>
          ...
          {!readOnly && (
            <label className="es-guest-toggle">
              ...
              <span className="es-toggle-label">
                {showExamsToGuests
                  ? `👁 ${t('visibleToGuests') || 'Visible to guests'}`
                  : `🔒 ${t('hiddenFromGuests') || 'Hidden from guests'}`}
              </span>
            </label>
          )}
      ...
      <div className="es-filters">
        ...
        <select className="es-select" value={filterDate} onChange={e => setFilterDate(e.target.value)}>
          ...
          {dates.map(d => <option key={d} value={d}>{fmtWithDay(d)}</option>)}
        </select>
        <input type="date" className="es-input" value={filterDate} onChange={e => setFilterDate(e.target.value)}
          style={{ padding:'5px 8px', fontSize:'0.78rem', maxWidth:148, cursor:'pointer' }} title={t('pickDate') || 'Pick a date'} />
        {hasFilters && <button className="es-btn-clear" onClick={clearFilters}>✕ {t('clearFilters') || 'Clear'}</button>}
        <div className="es-count">{filteredExams.length} {t('of') || 'of'} {exams.length} {exams.length !== 1 ? (t('exams')||'exams') : (t('exam')||'exam')}</div>
      </div>
        ...
        <div className="es-form-wrap">
          ...
          <div className="es-form-grid">
            <div className="es-field">
              ...
              <input className="es-input" placeholder={t('examSubjectPlaceholder')||'e.g. Mathematics'} value={form.subject} onChange={e => setForm(f=>({...f,subject:e.target.value}))} />
            </div>
            <div className="es-field">
          ...
          <div className="es-field es-field-full" style={{ marginBottom:14 }}>
            <label style={{ fontSize:'0.68rem', fontWeight:700, color:'#64748b', textTransform:'uppercase', letterSpacing:'.05em', marginBottom:6, display:'block' }}>
              ...
              <span style={{ color:'#94a3b8', fontWeight:400, textTransform:'none', marginLeft:6 }}>
                — {t('examGroupsHint')||'select all groups taking this exam'}
              </span>
            </label>
      ...
      {loading ? (
        ...
        <div className="es-empty">
          ...
          <div>{hasFilters ? (t('noExamsMatchFilters')||'No exams match the selected filters.') : (t('examNoData')||'No exams scheduled yet.')}</div>
          {hasFilters && <button className="es-btn-clear" style={{ marginTop:8 }} onClick={clearFilters}>✕ {t('clearFilters')||'Clear filters'}</button>}
          {!readOnly && !hasFilters && <div style={{ fontSize:'0.78rem', color:'#94a3b8', marginTop:4 }}>{t('addExamHint')||'Click "+ Add Exam" to create the first entry.'}</div>}
        </div>
      ) : (
        <div className="es-table-wrap">
          <table className="es-table">
            <thead>
              <tr>
                <th>{t('examDate')||'Date'}</th>
                <th>{t('examTime')||'Time'}</th>
                <th>{t('examSubject')||'Subject'}</th>
                <th>{t('examGroups')||'Groups'}</th>
                <th>{t('examTeacher')||'Examiner'}</th>
                <th>{t('examRoom')||'Room'}</th>
                <th>{t('examNotes')||'Notes'}</th>
                {!readOnly && <th></th>}
              </tr>
            </thead>
            <tbody>
                      ...
                      return (
                        <tr key={exam.id} className={conflictIds.has(exam.id) ? 'es-row-conflict' : ''}>
                          ...
                          <td className="es-td-time">
                            ...
                            <span className="es-td-dur">{exam.duration}{t('examMinutes')||'m'}</span>
                          </td>
                          <td className="es-td-subject">
                            {exam.subject}
                            {conflictIds.has(exam.id) && <span className="es-conflict-badge">⚠️ {t('conflict')||'conflict'}</span>}
                          </td>
                          <td>
                            <div className="es-td-groups">
                              {gNames.map(g => <span key={g} className="es-group-pill">{g}</span>)}
                              {gNames.length > 1 && <span className="es-group-joint">{t('joint')||'joint'}</span>}
                            </div>
                          </td>
                          ...
                          {!readOnly && (
                            <td>
                              <div className="es-td-actions">
                                <button className="es-action-btn tg" onClick={() => handleBroadcast(exam)} disabled={sending === exam.id} title={t('sendTelegram')||'Send Telegram'}>{sending === exam.id ? '⏳' : '📨'}</button>
                                <button className="es-action-btn edit" onClick={() => handleEdit(exam)} title={t('edit')||'Edit'}>✏️</button>
                                <button className="es-action-btn del" onClick={() => handleDelete(exam.id)} title={t('delete')||'Delete'}>🗑</button>
                              </div>
                            </td>
                          )}
                        </tr>
